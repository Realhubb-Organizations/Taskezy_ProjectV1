...
export default function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between relative overflow-x-hidden">
      ...
      {/* Header */}
      <header className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-8 py-4 sm:py-6 flex flex-wrap items-center justify-between gap-2">
        <Link href="/" className="flex items-center gap-2 text-slate-500 hover:text-slate-900 transition-colors">
          <ArrowLeft className="h-4 w-4" />
          <span className="text-sm font-semibold">Back to home</span>
        </Link>
        <span className="text-[10px] sm:text-xs text-slate-550 font-medium">STEP 1 OF 3: PLAN SELECTION</span>
      </header>

      {/* Main Grid */}
      <main className="relative z-10 flex-grow max-w-7xl w-full mx-auto px-4 sm:px-8 py-4 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
