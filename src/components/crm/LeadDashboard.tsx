...
export default function LeadDashboard() {
  ...
  // Sub-accounts leads mock data for popovers
  const metaSubAccounts = [
    { name: "Altura Facebook Leads Form", count: 182 },
    { name: "Granada Instagram Retargeting", count: 114 },
    { name: "Coimbatore Launch Lead Generation", count: 154 }
  ];

  const googleSubAccounts = [
    { name: "Search Ads - Coimbatore", count: 95 },
    { name: "Granada Display Network", count: 85 },
    { name: "Direct Search - Organic Landing", count: 60 }
  ];
  ...
  const totalLeadsSum = leads.length + 900; // Match baseline seed summary

  // Calculations matching Admin Cockpit
  const leadsToday = leads.filter(l => l.createdAtStr?.includes("06 Jul") || l.createdAtStr?.includes("07 Jul") || l.createdAtStr?.includes("08 Jul")).length || 7;
  const visitsToday = leads.filter(l => l.status === "Visit Schedule" || l.status === "Site Visit" || l.status === "Site Visit Scheduled" || l.status === "Meeting Scheduled").length || 8;
  const weekendVisits = 24;
  const monthBookings = leads.filter(l => l.status === "Booked" || l.status === "Booking Done" || l.status === "Booking Approved").length || 4;
