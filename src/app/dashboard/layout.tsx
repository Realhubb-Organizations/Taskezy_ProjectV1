...
function DashboardLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  // Dynamic context switching helper array matching our seeded users
  const simulationUsers = [
    { label: "GOD ADMIN", email: "admin@realhubb.in", role: "ADMIN" as Role, userId: "user-admin" },
    { label: "Finance Ops (Finance + HRMS)", email: "finance@realhubb.in", role: "FINANCE" as Role, userId: "user-fin-1" },
    { label: "Sales Mgr 1 (CRM + HRMS)", email: "sales.manager1@realhubb.in", role: "AGENT" as Role, userId: "user-mgr-sales" },
    { label: "Sales Mgr 2 (CRM + HRMS)", email: "sales.manager2@realhubb.in", role: "AGENT" as Role, userId: "user-mgr-tech" },
    { label: "Sales Mgr 3 (CRM + HRMS)", email: "sales.manager3@realhubb.in", role: "AGENT" as Role, userId: "user-mgr-mktg" },
    { label: "Sales Member (CRM + HRMS)", email: "sales1@realhubb.in", role: "AGENT" as Role, userId: "user-sales-1" },
    { label: "IT Member (HRMS only)", email: "it1@realhubb.in", role: "AGENT" as Role, userId: "user-it-1" },
    { label: "Mktg Member (HRMS only)", email: "marketing1@realhubb.in", role: "AGENT" as Role, userId: "user-mktg-1" }
  ];
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 text-slate-800">
      ...
      <div className="md:pl-64 flex flex-col flex-1 w-full min-h-screen">
        {/* Top Header Bar */}
        <header className="sticky top-0 z-10 flex-shrink-0 h-16 border-b border-slate-200 bg-white/70 backdrop-blur-md flex items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-bold text-brand-700 tracking-wide uppercase ml-1">{getActiveTabName()}</h1>
          </div>

          <div className="flex items-center gap-2 sm:gap-4">
            {/* Edge sync status */}
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200 bg-slate-50 text-xs">
              <button
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-xs font-bold text-slate-750 max-w-[200px] truncate sm:max-w-none">
        ...
        {isMobileMenuOpen && (
          <>
            ...
            />
            <div className="fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200 p-5 flex flex-col z-40 md:hidden">
        ...
        {/* Core Container */}
        <main className="flex-1 p-4 sm:p-6 lg:p-8 bg-slate-50/50 overflow-y-auto">
