...
export default function DashboardHome() {
  const { leads, properties, followupCalls, users, currentUser, invoices, reimbursements, activeSystem, timesheets, attendanceRecords } = useApp();
  ...
  if (activeSystem === "ADMIN") {
    ...
    const leadsToday = leads.filter(l => l.createdAtStr?.includes("06 Jul 2026") || l.createdAtStr?.includes("07 Jul 2026")).length || 14;
    const visitsToday = followupCalls.filter(c => c.type === "Site Visit" && c.status === "Upcoming").length || 8;
    const monthBookings = leads.filter(l => l.status === "Booking Done" || l.status === "Booking Approved").length || 4;
    ...
    const activeAgentsCount = users.filter(u => u.role === "AGENT").length || 5;
    ...
    return (
      <div className="space-y-8 pb-12 animate-fade-in">
        ...
        <MarketingOperations
          metaTotalSpend={243951}
          metaTodaySpend={8454}
          metaActiveCampaigns={33}
          googleTotalSpend={187320}
          googleTodaySpend={6210}
          googleActiveCampaigns={18}
        />
      </div>
    );
  }
      ...
      <div className="space-y-8 pb-12 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
            ...
            <p className="text-lg font-black text-slate-800 mt-1">15 users / 15 licensed</p>
            <span className="text-[9px] text-brand-600 font-bold mt-2">Utilization: 100%</span>
          </div>

          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Network Integration</span>
            <p className="text-lg font-black text-slate-800 mt-1">Meta CRM Webhook online</p>
            <span className="text-[9px] text-emerald-600 font-bold mt-2">Status: 200 OK</span>
          </div>
        </div>

        {/* Corporate Roster Listing */}
        <div className="glass-card p-6 rounded-2xl space-y-4">
          <h3 className="text-xs font-bold text-slate-700">Seeded User Directory (Realhubb Ventures)</h3>
    ...
    return (
      <div className="space-y-8 pb-12 animate-fade-in">
        <div>
          ...
          <p className="text-xs text-slate-500">Live Meta campaign spending and lead generation telemetry.</p>
        </div>

        {/* Campaign Metrics */}
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6 text-center text-xs bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div>
            <span className="text-slate-400 block font-semibold mb-0.5">Total Spend</span>
            <span className="text-lg font-black text-brand-700 font-mono">₹2,43,951</span>
          </div>
          <div className="border-l border-slate-200">
            <span className="text-slate-400 block font-semibold mb-0.5">Ingested Leads</span>
            <span className="text-lg font-black text-slate-850">426</span>
          </div>
          <div className="border-l border-slate-200">
            <span className="text-slate-400 block font-semibold mb-0.5">Avg CPL</span>
            <span className="text-lg font-black text-brand-700 font-mono">₹573</span>
          </div>
          <div className="border-l border-slate-200">
            <span className="text-slate-400 block font-semibold mb-0.5">Impressions</span>
            <span className="text-lg font-black text-slate-850">1.3M</span>
          </div>
          <div className="border-l border-slate-200">
            <span className="text-slate-400 block font-semibold mb-0.5">CTR (Click Rate)</span>
            <span className="text-lg font-black text-emerald-650 font-mono">1.18%</span>
          </div>
        </div>

        {/* Detailed Marketing Campaigns */}
        <div className="glass-card p-6 rounded-2xl space-y-4">
          <h3 className="text-xs font-bold text-slate-700">Meta Ad Set Performance breakdown</h3>
          <div className="border border-slate-205 rounded-xl overflow-hidden bg-white shadow-sm">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[9px] uppercase font-bold text-slate-500 tracking-wider">
                  <th className="p-3">Rank</th>
                  <th className="p-3">Ad Name / Segment</th>
              ...
              <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                {[
                  { rank: "#1", name: "Realhubb B.Granada Kadugodi Road Ele.Ai Loc", leads: 53, spend: "₹8,454", ctr: "2.22%" },
                  { rank: "#2", name: "RealHubb CKPC HOH Hosur UP Loc", leads: 47, spend: "₹11,173", ctr: "1.48%" },
                  { rank: "#3", name: "RealHubb CKPC HOH Hosur Vid Loc", leads: 43, spend: "₹13,487", ctr: "1.27%" }
                ].map((ad, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    <td className="p-3 text-slate-400">{ad.rank}</td>
                    <td className="p-3 text-slate-805 truncate max-w-[200px]">{ad.name}</td>
                    <td className="p-3 font-black text-slate-800">{ad.leads}</td>
                    <td className="p-3 text-slate-600 font-mono">{ad.spend}</td>
                    <td className="p-3 text-right text-emerald-600 font-mono font-bold">{ad.ctr}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div className="space-y-8 pb-12 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm text-center flex flex-col justify-between">
          ...
          <p className="text-2xl font-black text-emerald-650 mt-1">94.2%</p>
          <span className="text-[9px] text-slate-450 mt-2 block">July geofenced logs</span>
        </div>
      </div>
  ...
  const totalCallsCount = scopedCalls.length + 30; // Add 30 completed simulation calls for realistic metrics
  const completedCallsCount = 30 + scopedCalls.filter(c => c.status === "Completed").length;
      ...
      <div className="glass-card p-6 rounded-2xl space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-6 text-center">
          <div className="space-y-1">
            ...
            <span className="text-[9px] text-slate-450 block">{totalCallsCount - 5} Unique prospects</span>
          </div>
          ...
          <div className="space-y-1 border-l border-slate-200">
            ...
            <p className="text-xl font-black text-slate-800">
              {isSalesMember ? "1h 14m 20s" : "6h 2m 58s"}
            </p>
            <span className="text-[9px] text-slate-450 block">Avg 2m 8s / call</span>
          </div>
          ...
          <div className="space-y-1 border-l border-slate-200">
            ...
            <span className="text-[9px] text-emerald-600 font-bold block">
              {Math.round((completedCallsCount / totalCallsCount) * 100)}% Success
            </span>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="glass-card p-6 rounded-2xl space-y-4">
            ...
            <div className="space-y-2 max-h-44 overflow-y-auto pr-1 text-xs">
              {Array.from(new Set(scopedLeads.map(l => l.property || "Vanantara"))).length === 0 ? (
                <div className="text-slate-400 italic">No project data.</div>
              ) : (
                Array.from(new Set(scopedLeads.map(l => l.property || "Vanantara"))).map((propName, idx) => {
                  const count = scopedLeads.filter(l => (l.property || "Vanantara") === propName).length;
                  return (
